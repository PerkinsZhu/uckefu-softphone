...
var uKeFuSoftPhone = {
	...
	login(extno , extpass){
		var socket = new JsSIP.WebSocketInterface('ws://'+ws_address+':'+ws_port);
		var config = {
...
*/
		...
		softPhoneUA.on('progress',function (e){
			console.log('call is in progress');
		});
		softPhoneUA.on('failed',function (e){
			console.log('call failed with cause: '+ e.data.cause);
		});
		softPhoneUA.on('ended',function (e){
			console.log('call ended with cause: '+ e.data.cause);
		});
		softPhoneUA.on('confirmed',function (e){
			console.log('call confirmed');
		});
	...
	invite:function(pnumber){
		console.log("===invite===")
		currentSession = softPhoneUA.call(pnumber+"@"+ws_address , uKeFuSoftPhone.getOptions());
